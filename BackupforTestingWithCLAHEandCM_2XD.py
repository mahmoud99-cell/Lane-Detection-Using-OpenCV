...
def image_process(img):
    ...
    """
    #I tried to use sobel + otsu fk worst output ever
    ...
    combined_sobel_colors_img = combined_sobel_colors(cl1, 15, (25, 225))
    
    ###########################################################################################
    #print('combined_sobel_colors_img',combined_sobel_colors_img.shape , combined_sobel_colors_img.dtype)
    #print('color_image',color_image.shape , color_image.dtype)
    #####################################################################################
    #fking error had to convetr to unit8
    color_image=threshold_color_space(img)
    print('color image before :',color_image.dtype)
    color_image = color_image.astype(np.uint8)
    print('color image after: ',color_image.dtype)
    lines_frame=hough_lines(color_image,2,np.pi/180,100,40,5) #houghed frame and drawn this outputs

    #print('color_image',color_image.shape , color_image.dtype)
    #making binary image of result
    #grayImage = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #changed sobal colormap to binary
    #(thresh, blackAndWhiteImage) = cv2.threshold(grayImage, 120, 230, cv2.THRESH_BINARY)
    #display(blackAndWhiteImage, opening_color, 'blackAndWhiteImage', 'opening_color thresh', 14, 7, cmap='gray')
    #####################################################################################
    #reducing noise 

    #the combination of color & sobel
    
    #print(lines_frame.shape , lines_frame.dtype, ':lines_frame' )
    #print(combined_sobel_colors_img.shape , combined_sobel_colors_img.dtype, ':combined_sobel_colors_img' )    
    #print(img.dtype,img.shape,':img')
    #print(lines_frame.shape,lines_frame.dtype)
    #print(combined_sobel_colors_img.shape,combined_sobel_colors_img.dtype)    
    
    #lines_frame = combined_sobel_colors(lines_frame, 15, (25, 225))
    grayImage = cv2.cvtColor(lines_frame, cv2.COLOR_BGR2GRAY) #changed sobal colormap to binary
    (thresh, blackAndWhiteImage_houghed_color) = cv2.threshold(grayImage, 25, 225, cv2.THRESH_BINARY)
    

    #print(lines_frame.shape,lines_frame.dtype)
    #print(combined_sobel_colors_img.shape,combined_sobel_colors_img.dtype)
    #print('lines from sobel:',lines_frame.shape)
    #print('color_image:',color_image.shape)
    #grayImage = cv2.cvtColor(lines_frame, cv2.COLOR_BGR2GRAY)
    ...
    #all ways of sum
    print('color image after: ',combined_sobel_colors_img.shape)
    
    #channel by channel i think its working best option now
    combined_sobel_colors_img[:,1]=(combined_sobel_colors_img[: ,1] + blackAndWhiteImage_houghed_color[: ,1]) 
    combined_sobel_colors_img[:,2]=(combined_sobel_colors_img[: , 2]+blackAndWhiteImage_houghed_color[: , 2]) 
    
    
    #region_of_interest(blackAndWhiteImage_houghed_color) , blackAndWhiteImage_houghed_color.shape )
    #combined_sobel_colors_img= cv2.add(combined_sobel_colors_img , blackAndWhiteImage_houghed_color)
    #combined_sobel_colors_img = cv2.addWeighted(  combined_sobel_colors_img, 0.9,  blackAndWhiteImage_houghed_color,0.1 , 0 )
    #combined_sobel_colors_img= combined_sobel_colors_img+ blackAndWhiteImage_houghed_color
    #final=cv2.bitwise_or(combined_sobel_colors_img,blackAndWhiteImage_houghed_color)
    
    #contours, hierarchy = cv2.findContours(final, cv2.RETR_CCOMP , cv2.CHAIN_APPROX_SIMPLE)
    #cv2.drawContours(img, contours, -1, (0,255,0), 3)
    """display(combined_sobel_colors_img,
            blackAndWhiteImage_houghed_color,
            'sobel image',
            'HOUGHED Color  image',
            14,
            7,
            ...
            n = 3)
    """
    #final= region_of_interest(final)
    
    
    return combined_sobel_colors_img, Minv
